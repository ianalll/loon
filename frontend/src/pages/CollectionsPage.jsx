...
const CollectionsPage = () => {
  ...
  const allAvailableSizes = () => {
    ...
    Object.values(allSizes).forEach(sizes => {
      sizes.forEach(size => {
        if (size.quantity > 0) {
  ...
  return (
    <>
      <CollectionsContainer>
        <ContentWrapper>
          <PageTitle>КОЛЛЕКЦИИ LOON</PageTitle>
          <Description>ВДОХНОВЛЯЮЩИЕ ОБРАЗЫ ДЛЯ КАЖДОГО ДНЯ</Description>
          ...
          <FilterSection>
            ...
            <FilterSelect value={selectedCollection} onChange={(e) => setSelectedCollection(e.target.value)}>
            ...
            <FilterSelect value={selectedSize} onChange={(e) => setSelectedSize(e.target.value)}>
            ...
            <PriceGroup>
              ...
              <PriceInput 
                type="number" placeholder="999999" value={maxPrice}
          ...
          {filteredProducts.length === 0 ? (
            <EmptyMessage>Товары не найдены</EmptyMessage>
          ) : (
            <>
              <ResultCount>Найдено товаров: {filteredProducts.length}</ResultCount>
              <ProductsGrid>
                {filteredProducts.map(product => {
                  const sizes = allSizes[product.id] || [];
                  ...
                  return (
                    <ProductCard as={Link} to={`/product/${product.id}`}>
                      ...
                      <ProductInfo>
                        ...
                        <ButtonGroup>
                          <ActionButton onClick={() => addToFavorites(product.id)}>В ИЗБРАННОЕ</ActionButton>
                          <ActionButton onClick={() => openSizeModal(product)}>В КОРЗИНУ</ActionButton>
                        </ButtonGroup>
                      </ProductInfo>
                    </ProductCard>
                  );
      ...
      {showSizeModal && selectedProduct && (
        <ModalOverlay onClick={() => setShowSizeModal(false)}>
          <ModalContent onClick={e => e.stopPropagation()}>
            ...
            <ModalButtons>
              <ModalConfirmButton onClick={addToCart}>Добавить в корзину</ModalConfirmButton>
              <ModalCancelButton onClick={() => setShowSizeModal(false)}>Отмена</ModalCancelButton>
            </ModalButtons>
          </ModalContent>
        </ModalOverlay>
      )}
