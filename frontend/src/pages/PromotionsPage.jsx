...
const PromotionsPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const promotionOnly = productsRes.data.filter(p => p.is_promotion === true);
      setProducts(promotionOnly);
      setFilteredProducts(promotionOnly);
      setCollections(collectionsRes.data);
      
      const sizesPromises = promotionOnly.map(product => 
        axios.get(`http://localhost:5000/api/products/${product.id}/sizes`).catch(() => ({ data: [] }))
      );
      const sizesResults = await Promise.all(sizesPromises);
      
      const sizesMap = {};
      promotionOnly.forEach((product, index) => {
      ...
      <PromotionsContainer>
        <ContentWrapper>
          ...
          <SearchSection>
            <SearchInput 
              type="text"
              placeholder="Поиск товаров по акции..."
              value={searchTerm}
          ...
          {filteredProducts.length === 0 ? (
            <EmptyMessage>Товары по акции не найдены</EmptyMessage>
          ) : (
            <>
              <ResultCount>Найдено товаров по акции: {filteredProducts.length}</ResultCount>
              <ProductsGrid>
